...
import { removeFavorite } from "../../../store/favourite";
import { useCallback, useMemo} from "react";

const ArketRemoveButton = ({onClick}) => (
    <button className="removeBtn arket" onClick={onClick}>
...
const StoriesRemoveButton = ({onClick}) =>(
    <button className="removeBtn stories" onClick={onClick}>
        <svg class="IconWrapper-module--icon__e4eqD RemoveButton-module--removeButtonIcon__OFRO8 IconWrapper-module--small__k8lhm" viewBox="0 0 24 24" focusable="false">
            <g fill="none">
                <path d="M0 0h24v24H0z"></path>
                <path d="m19.462 4.5-1.157 15.61a1.496 1.496 0 0 1-1.496 1.39H7.191a1.496 1.496 0 0 1-1.496-1.39L4.538 4.5h14.924Z" stroke="#000" fill="#F3F3F3"></path>
                <path stroke="#000" d="M12 8v10M8 8l1 10M16 8l-1 10M3.5 4.5h17v1h-17z"></path>
                <path d="m14 2.5.5 2h-5V3l4.5-.5Z" stroke="#000" fill="#FFF"></path>
            </g>
        </svg>
    </button>
...
const FavElement = ({id, image, description, isSelectable, isClicked,brand}) => {
    const dispatch = useDispatch();
    const onRemove = ()=>{dispatch(removeFavorite({id}))}
    return (<li>
    <div class='listGrid__element'>
        <ProductImage image={image}>
        {
           brand === "arket" ? (
            <ArketRemoveButton onClick={()=>{
                dispatch(removeFavorite({id}))}}/>
           ): brand === "stories" ?
            <StoriesRemoveButton onClick={onRemove}/>
            : ""
        }
        </ProductImage>
        <ProductDescription description={description} brand={brand}/>
